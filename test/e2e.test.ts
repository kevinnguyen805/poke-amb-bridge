...
// End-to-end acceptance tests over the already-unit-tested spine modules: they wire
// mint -> ingress -> egress and assert the system-level guarantees from the plan's User Flows.
...
/** The bcrw `body` is `poke%3A<token>`; the user's tap delivers the decoded `poke:<token>`. */
function bodyFromBcrw(bcrwUrl: string): string {
  return decodeURIComponent(bcrwUrl.split("body=")[1]!);
}

describe("E2E spine", () => {
  it("Flow 1 — Share-to-Poke: shared URL → linked, labeled AMB reply, NO account bound, NO raw handle stored", () => {
    const w = world();
    const minted = mint(
      { tokens: w.tokens, businessUuid: UUID, clock },
      { kind: "scoped-key" }, // the Shortcut's public scoped key
    ...
    expect(w.continuity.get("urrn_user_1")?.pokeAccountId).toBeUndefined();
    expect(w.msp.sent.every((s) => s.msg.aiLabeled && s.msg.escalation && s.msg.applePresentation)).toBe(true);
    expect("handle" in w.tokens.get(minted.token)!).toBe(false); // PII minimization
  });

  it("Flow (consented binding) — a session + consent token binds the Poke account across the first AMB message", () => {
    const w = world();
    const minted = mint(
      { tokens: w.tokens, businessUuid: UUID, clock },
    ...
    expect(w.continuity.get("urrn_user_2")?.pokeAccountId).toBe("acct_99");
    expect(w.continuity.get("urrn_user_2")?.consentRef).toBe("consent_logged");
  });

  it("Single-use end to end: the same shared link presented by a SECOND opaque ID is rejected (no takeover)", () => {
    const w = world();
    const minted = mint(
      { tokens: w.tokens, businessUuid: UUID, clock },
      { kind: "scoped-key" },
    ...
    expect(resolve(deps, { opaqueId: "urrn_owner", body, receivedAt: clock.now().toISOString() }).branch).toBe("bound");
    expect(resolve(deps, { opaqueId: "urrn_attacker", body, receivedAt: clock.now().toISOString() }).branch).toBe(
      "foreign-reject",
    );
    expect(w.continuity.get("urrn_attacker")?.pokeAccountId).toBeUndefined();
  });
});
