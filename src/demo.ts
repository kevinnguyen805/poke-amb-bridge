 ...
 * Shows the four plan User Flows end to end against in-memory stores + a mock MSP.
 */
import {
  mint,
  resolve,
...
async function main() {
  ...
  line("FLOW 1  Share-to-Poke (Shortcut → AMB, anonymous)");
  {
    ...
    const out = await resolve(w.deps, {
    ...
    line(`  pii   → token store holds raw handle? ${"handle" in (await w.tokens.get(minted.token))!}\n`);
  }
  ...
  line("FLOW 2  Consented binding (session + logged consent → account-bound)");
  {
    ...
    line(`  bind  → accountBound=${out.accountBound} pokeAccountId=${(await w.continuity.get("urrn_user_2"))?.pokeAccountId}\n`);
  }
  ...
  line("FLOW 3  Single-use: a second opaque ID replaying the same link is rejected");
  {
    ...
    const a = await resolve(w.deps, { opaqueId: "urrn_owner", body, receivedAt: clock.now().toISOString() });
    const b = await resolve(w.deps, { opaqueId: "urrn_attacker", body, receivedAt: clock.now().toISOString() });
  ...
  line("FLOW 4  Cold start (tokenless open_chat, e.g. QR / poke.com)");
  {
    ...
    const out = await resolve(w.deps, { opaqueId: "urrn_user_3", body: "hi what can you do", bizGroupId: "qr", receivedAt: clock.now().toISOString() });
